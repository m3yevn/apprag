    ...
    return __generator(this, function (_a) {
        switch (_a.label) {
            ...
            case 1:
                licenseFile = _a.sent();
                _a.label = 2;
            case 2: return [2 /*return*/, __assign(__assign({}, packageInfo), { name: packageInfo.name
                        ? packageInfo.name.charAt(0).toUpperCase() +
                            packageInfo.name.slice(1)
                        : "This project name", funFacts: packageInfo.funFacts
                        ? renderList(packageInfo.funFacts, " - {}")
                        : "This project is awesome!", badges: packageInfo.badges
                        ? renderList(packageInfo.badges, "{}")
                        : "This project is well tested!", techStacks: packageInfo.techStacks
                        ? renderList(packageInfo.techStacks, " - {}")
                        : "This project is using awesome tech stacks!", publicUrl: packageInfo.publicUrl || "This project is not published to public!", screenshots: packageInfo.screenshots
                        ? renderList(packageInfo.screenshots, ' - <img src="{}" />')
                        : "This project does not have screenshots available.", scripts: packageInfo.scripts && structureScripts(packageInfo.scripts).length
                        ? renderList(structureScripts(packageInfo.scripts), "{}")
                        : "This project does not have scripts to run.", bugUrl: packageInfo.bugs && packageInfo.bugs.url
                        ? packageInfo.bugs.url
                        : "Visit the repository to open bug reports and issues", license: packageInfo.license
                        ? licenseExists
                            ? licenseFile
                            : packageInfo.license
                        : "This project does not have a license.", dependencies: packageInfo.dependencies &&
                        ...
                        : "This project does not have dev dependencies", animations: packageInfo.animations
                        ? renderList(packageInfo.animations, '<img src="{}"')
                        : '<img src="https://cdn.dribbble.com/users/2401141/screenshots/5487982/developers-gif-showcase.gif"/>', footer: packageInfo.footer || "Happy Coding!" })];
        }
    });
}); };
